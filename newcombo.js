...
function nc_open()
{
	...
	$("float_newcombo").style.display = "block";
}

function nc_close()
{
	$("float_newcombo").style.display = "none";
}
...
function setComboCycle()
{
	if(hitObjectArray.length == 0) { output("No object!"); return 0; }
	var a = $("newcombo_cycletext").value.split(",");
	var arr = [];
...
function setComboCycle2()
{
	...
	for(var i=0;i<hitObjectArray.length;i++)
	{
		...
		if(obj.time >= dTimeStart && obj.time <= dTimeEnd)
		{
			...
			{
				var choices = arr.filter(n => n != history[0] && n != history[1]);
				var chosen = choices[Math.floor(choices.length * Math.random())];
				setComboHere(obj,chosen);
				locSpinBuff = 0;
