...
function parseDiffdata(diffData)
{
	...
	diffSettings.HD = Math.min(parseInt(diffData.match(/HPDrainRate:([0-9]+)/i)[1]),10);
	diffSettings.CS = parseInt(diffData.match(/CircleSize:([0-9]+)/i)[1]);
	diffSettings.OD = parseInt(diffData.match(/OverallDifficulty:([0-9]+)/i)[1]);
	...
	diffSettings.AR = AR;
	APC_ScaleTime = AR_Correspondence[AR];
	HC_StandingTime = AR_Correspondence[AR];
	APC_FadeInTime = Math.round(AR_Correspondence[AR] / 3);
	HC_FadeInTime = Math.round(AR_Correspondence[AR] / 3);
	HC_FadeIn2Time = Math.round(AR_Correspondence[AR] / 3);
	HC_FadeIn2Dur = Math.round(AR_Correspondence[AR] / 15);
	//HC_ExplosionTime = Math.round(AR_Correspondence[AR] / 8);
	//HC_FadeOutTime = Math.round(AR_Correspondence[AR] / 4);
	circleScaling = CS_Correspondence[diffSettings.CS] * (0.85);
}
...
function wwww(k)
{
	...
	switch(k)
	{
		...
		output("24whistle修改完成");
		break;
		case 6:
		diffname_buff("24whis_clap");
		output("24 whistle/clap修改完成");
		break;
		case 7:
		diffname_buff("24clap_whis");
		output("24 clap/whistle修改完成");
		break;
		case 8:
...
function init()
{
	...
	clear_buff();
	color_dark();
	output("osu id: [CSGA]Ar3sgice 2012.3.9-2015.2.19");
	output("osu! 批量音效工具 v0.7");
}
window.onload = init;