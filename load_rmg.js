...
function sv2()
{
	if(is_map_loaded || $("load_map_file").value && $("load_map_file").value.length > 2)
	{
		breakmap5();
		var p = save_file();
		load_map();
...
function load_web()
{
	var loc = $("load_map_web_input").value;
	if(!loc.length)
	{
		return false;
	}
	...
	ajax.onreadystatechange = function() { if(ajax.readyState == 4) { load_web_callback(ajax.responseText, ajax.status); } };
	...
	$("load_map").style.display = "none";
		$("loading").innerHTML += "File not found, <a href='javascript:reset_loader();'>reload</a> <br />";
		return;
	}
	$("loading").style.display = "block";
	$("load_map").style.display = "none";
	$("loading").innerHTML = "Reading from " + loc + " ... <br /> ";
}
...
function reset_loader()
{
	...
	$("load_map").style.display = "block";
}

function load_web_callback(rs, sts)
{
	if(sts == 404)
	{
		$("loading").innerHTML += "404, <a href='javascript:reset_loader();'>reload</a> <br />";
		return;
	}
	if(!rs.length)
	{
		$("loading").innerHTML += "Read failed, file is empty, <a href='javascript:reset_loader();'>reload</a> <br />";
	}
	else
	{
		$("loading").innerHTML += "Loaded!<br />";
		load_map(0, rs);
		// displayGenerationButton();
	}
}

function displayGenerationButton()
{
	$("loading").style.display = "none";
	$("save_map").style.display = "block";
	$("load_map").style.display = "none";
}
...
function init()
{
	...
	{
		var lhs = location.href.toString();
		if(lhs.match(/(\?|&)m=(.*)(&|$)/i))
		{
			var mapid = lhs.match(/(\?|&)m=(.*?)(&|$)/i)[2];
			$("load_map_web_input").value = mapid;
			load_web();
		}
	}
}
